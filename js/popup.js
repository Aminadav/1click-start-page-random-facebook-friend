chrome.permissions.contains({origins: ['https://*.facebook.com/*']},function(status){
if(status){
    $('body').addClass('wide').append($('<div class="grant-perm">' + chrome.i18n.getMessage('refresh_explain') + '<button type="button" class="refresh-friends btn">' + chrome.i18n.getMessage('refresh_friends') + '</button></div>'))
    $('.refresh-friends').click(function(){
